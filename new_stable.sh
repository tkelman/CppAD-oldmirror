...
#
# check in for files that are automatically changed
sed -i svn_commit.sh \
	-e 's|\(log_entry="\)|\1Changes generated by new_stable.sh|' \
	-e 's|\(change_list="\)|\1\n\tnew_stable.sh|' \
	-e 's|\(change_list="\)|\1\n\tcppad/config.h|' \
...
echo "6: Commit changes in stable/$stable_version using"
echo "      stable/$stable_version/svn_commit.sh"
