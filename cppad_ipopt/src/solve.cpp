...
$section Use CppAD and Ipopt to Solve a Nonlinear Programming Problem$$

$head Syntax$$
$codei%# include "cppad_ipopt_solve.hpp"
%$$
$codei%cppad_ipopt_solution %solution%;
%$$
$codei%cppad_ipopt::solve%(
	%nf%, %xi%, %xl%, %xu%, %gl%, %gu%, %fg_eval%, %options%, %result%
)%$$

$head Purpose$$
The function $code cppad_ipopt::solve$$ solves nonlinear programming
...
$href%
	...
	Ipopt
%$$
optimizer CppAD for the derivative and sparsity calculations. 
...
so as to minimize the maximum, with resptect to $latex i$$, of $latex nd(i)$$.

$head xi$$
The argument $icode xi$$ has prototype
$codei%
	const CppAD::vector<double>& %xi%
%$$
...
used by $icode fg_eval$$ to assing the elements of $icode fg$$ 
does not depend on the value of the argument $icode x$$.
...
$codei%
	cppad_ipopt::solution_status %result%.status
%$$
...
$codei%
	CppAD:::vector<double> %result%.lambda
%$$
and its size is equal to $icode ng$$.
